...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`announcements_id\` text(36) REFERENCES announcements(id);`,
  );
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`websites_id\` text(36) REFERENCES websites(id);`,
  );
  await db.run(
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "events_id", "event_series_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "events_id", "event_series_id" FROM \`payload_locked_documents_rels\`;`,
  );
